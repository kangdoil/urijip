...
PRD(`docs/PRD-우리집.md` v0.9) 기준 개발 로드맵. 단계를 마칠 때마다 체크 표시를 갱신한다.
...
- [x] ⑤ 결과 화면 지도+바텀시트 리뉴얼 (`src/components/result-map-sheet.tsx`) — 전체화면
      카카오맵 + 시군구 칩 스와이프 + 활성 칩 상위 3동 지도 핀. `get_matches`/
      `get_fallback_matches`가 이제 `areas.lat/lng`도 반환하도록 마이그레이션 추가
      (`20260714000000_area_coords_in_matches.sql`). 칩 전환·리스트 갱신·폴백 상태(A/B
      색상 구분 핀)까지 실 세션으로 E2E 검증 완료. **지도 렌더링만 미검증** — 카카오맵
      JS SDK가 `403 App(우리집) disabled OPEN_MAP_AND_LOCAL service`로 막힘, 앱을
      비즈 앱으로 전환해야 풀림 (PRD §9). 사용자 확인 후 최종 스크린샷 검증 필요
