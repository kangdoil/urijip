-- =============================================================
-- 콜드 스테이션 회복 v3: 순위 완화 사다리에 "전체 해제" 안전망 단계를 추가한다.
--
-- 배경: 지금 사다리(get_concession_matches)는 2단계부터 "2순위 조건 해제"만
-- 지원한다(_priority_hard_ok가 항상 priority<=1로만 완화). 그런데 1순위
-- 조건 자체가 병목이면(예: "년식"은 전체 지역의 8%만 만족 — area_stats 실측)
-- 어떤 통근/예산 확장이나 2순위 해제로도 후보가 안 열려서 사다리가 6단계를
-- 전부 돌아도 total_count=0으로 끝나고, 팁 카드에 배지도 칩도 없이 일반
-- 문장 하나만 뜬다(사용자 관점에선 "조율 추천이 안 뜬다"로 보임).
--
-- 이 마이그레이션은 _priority_hard_ok의 완화 플래그를 boolean(2순위만)에서
-- int min_priority(2=아무것도 안 풂/1=2순위 해제/0=전부 해제)로 일반화하고,
-- 사다리 맨 끝에 min_priority=0(순위 하드필터 완전 해제, 점수 정렬만) 단계를
-- 하나 추가한다. 기존 0~4단계 동작은 전혀 바뀌지 않는다 — 그 단계들로 이미
-- 풀리는 세션은 지금처럼 동작하고, 지금까지 "빈 카드"로 끝나던 세션에만 이
-- 안전망이 추가로 개입한다.
--
-- apply_concession은 relieved_all일 때 순위(participant_conditions)는 건드리지
-- 않는다 — get_matches는 항상 1·2순위를 하드 체크하므로, 순위 자체를 "완전히
-- 안 지켜도 됨"으로 영구 반영할 방법이 스키마에 없다(그러려면 get_matches의
-- 하드필터에 별도 예외 플래그가 필요 — 더 큰 변경이라 이번 범위 밖). 그래서
-- 이 케이스는 통근·예산 widen만 반영하고, 그래도 여전히 0곳이면 사용자가
-- 조율하기에서 직접 순위를 조정하게 안내한다(이전부터 있던 경로).
-- =============================================================

drop function if exists public._priority_hard_ok(uuid, jsonb, boolean);
create or replace function public._priority_hard_ok(
  pid uuid, satisfied jsonb, min_priority int default 2
...
drop function if exists public._concession_ladder_step(uuid, text, text, int, bigint, boolean, boolean);
...
-- =============================================================
-- get_concession_matches: 사다리 배열 맨 끝에 min_priority=0(전체 해제) 단계
-- 하나만 추가한다. 그 앞 0~4단계 로직/widen 값은 그대로다.
-- =============================================================
...
begin
  ...
  --     맨 끝 step 5가 이번에 추가한 "순위 전체 해제" 안전망이다. =====
  ...
  for i in 0..step_count - 1 loop
    ...
    if i = step_count - 1 then
      -- 마지막 단계(순위 전체 해제 포함)까지 전부 0곳 — ladder_step은 null(실패)로 남긴다.
