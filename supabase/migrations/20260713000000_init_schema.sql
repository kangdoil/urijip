...
-- =============================================================
-- 초대 프리뷰 & 참여 현황 (이번 세션 신설)
-- schema.sql v0.2에는 없던 함수 2개. "프로젝트 기반 공사" 세션에서
-- 초대 링크 동적 OG 태그와 대기 화면 참여 확인을 구현하려면
-- 세션 멤버가 되기 전(B가 링크만 열어본 시점) / 조건을 아직 완료하지
-- 않은 시점에도 "누가·몇 명" 정도는 공개할 수 있는 통로가 필요하다.
-- 두 함수 모두 실제 조건 데이터(commute_max_min, budget_max_krw,
-- participant_conditions)는 절대 반환하지 않는다 — PRD의
-- "상대 입력 완료 전 조건 비공개" 원칙은 그대로 유지된다.
-- =============================================================

-- 초대 링크 프리뷰: 세션 멤버가 아니어도(비로그인 크롤러 포함) 호출 가능.
-- OG 태그·초대 페이지에 "OO님이 초대했어요"를 보여주기 위함.
...
-- 세션 참여 현황: 세션 멤버만 호출 가능. 인원 수·역할만 반환하고
-- 조건 값은 담지 않는다 (대기 화면에서 "상대가 참여했어요" 표시용).
